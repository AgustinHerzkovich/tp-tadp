...
module Invariant
    def invariant(&invariante)
        proc_invariante = proc do |instancia, method_name| # Añado el parametro "instancia" al bloque para saber en que contexto ejecutar el bloque, y el nombre del metodo pora matar la recursividad
            ...
            unless ultimo_metodo == method_name # Evitamos recursividad por si el bloque llama un getter
                ...
                unless instancia.instance_exec(&invariante)
                    raise "Excepcion: Objeto en estado invalido"
                end
            end
        end
        ...
        metodos_a_modificar.each do |method_name| # Por cada simbolo
